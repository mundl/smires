...
contrib_ts <- function(country, contact, ts = NA, publish = FALSE, comment = NA, meta = FALSE)
{
...
Currently ``r length(unique(dat$country))`` countries have submitted time series of which  ``r length(unique(dat$country[dat$include]))`` countries agreed on indluding the data into the R-package.
...
library(ggplot2)
ggplot(poly, aes(long, lat, group = id, fill = col)) + 
