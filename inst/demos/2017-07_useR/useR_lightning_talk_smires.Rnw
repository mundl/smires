...
\begin{frame}[fragile]{Functions}
	...
	\begin{tabular}{@{}ll}
		\verb|metric()| & continuous time series \\
		\verb|smires()| & binary time series, e.g. for intermittent rivers
	\end{tabular}
...
<<ma-discharge>>=
metric(balder, major = 244, fun_major = max)
@
...
library(lubridate)
balder <- check_ts(balder[year(balder$time) < 1979, ])
@

	\footnotesize
<<binary, fig=TRUE, include=FALSE, width=7.437506, height=2.343752>>=
smires(balder, plot = T) %>% head(3)
@
...
smires(balder, threshold = 0.2, plot = T) %>% head(3)
@
...
<<>>=
smires(balder, minor = seasons, fun_minor = max)
@
\end{frame}
